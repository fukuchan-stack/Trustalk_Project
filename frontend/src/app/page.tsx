...
export default function Home() {
  ...
  const fetchHistory = async () => {
    try {
      // ★ 修正点: APIのURLをプロキシへの相対パスに変更
      const res = await fetch('/api/proxy/api/history');
      if (!res.ok) {
        throw new Error(
          '履歴の取得に失敗しました。バックエンドサーバーが起動しているか確認してください。',
        );
      }
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res
          .json()
          .catch(() => ({ error: 'サーバーから有効な応答がありません。' }));
        throw new Error(
          errorData.error ||
            'アップロードに失敗しました。バックエンドサーバーが起動しているか確認してください。',
        );
      }
      const data = await res.json();
      if (data.error) {
              ...
              <ul className='list-disc pl-5 space-y-2'>
                {analysisResult.todos.map((todo, index) => (
                  <li key={index} className='text-gray-700'>
                    <strong>{todo.assignee || '未割り当て'}:</strong>{' '}
                    {todo.task}
                    {todo.due_date && (
                ...
                <tr key={log.id} className='hover:bg-gray-50'>
                  ...
                  <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                    <p className='text-gray-900 whitespace-pre-wrap'>
                      {log.generated_answer.substring(0, 100)}...
                    </p>
                  </td>
                  <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                    <span
                      className={`relative inline-block px-3 py-1 font-semibold leading-tight ${log.final_judgement === 'O' ? 'text-green-900' : 'text-red-900'}`}
                    >
                      <span
                        aria-hidden
                        className={`absolute inset-0 ${log.final_judgement === 'O' ? 'bg-green-200' : 'bg-red-200'} opacity-50 rounded-full`}
                      ></span>
