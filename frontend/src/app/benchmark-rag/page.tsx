// frontend/src/app/benchmark-rag/page.tsx
...
interface RagFinalScores {
  average_faithfulness: number;
  average_answer_relevancy: number;
}
interface RagBenchmarkResult {
  results_by_question: RagResultByQuestion[];
  final_scores: RagFinalScores;
}
...
export default function BenchmarkRagPage() {
  ...
  const handleCheckboxChange = (index: number) => {
    setSelectedQuestions(prev => ({ ...prev, [index]: !prev[index] }));
  };
  const handleSelectAllQuestionsChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const isChecked = event.target.checked;
    const newSelection: Record<number, boolean> = {};
    qaPairs.forEach((_, index) => {
      newSelection[index] = isChecked;
    });
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const selectedIndices = Object.keys(selectedQuestions)
      .filter(index => selectedQuestions[parseInt(index)])
      .map(index => parseInt(index));

    if (selectedIndices.length === 0) {
      setError('少なくとも1つの質問を選択してください。');
      return;
    }
    ...
    <main className="flex min-h-screen flex-col items-center p-4 sm:p-8 bg-gray-50">
      <div className="w-full max-w-6xl">
        ...
        <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200 mb-12">
          ...
          <p className="text-gray-600 mb-6">質問応答データセットとコンテキスト（知識源）のCSVファイルをアップロードし、選択したモデルのRAG性能を評価します。</p>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label htmlFor="context-file-upload" className="block text-sm font-medium text-gray-700 mb-2">2. 文脈CSVファイルを選択</label>
              <input id="context-file-upload" type="file" onChange={(e) => e.target.files && setContextFile(e.target.files[0])} accept=".csv" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
               <p className="mt-1 text-xs text-gray-500">必須カラム: `context`</p>
            </div>
            <div>
              <label htmlFor="qa-file-upload" className="block text-sm font-medium text-gray-700 mb-2">3. 質問・回答CSVファイルを選択</label>
              <input id="qa-file-upload" type="file" onChange={handleQaFileChange} accept=".csv" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
              <p className="mt-1 text-xs text-gray-500">必須カラム: `question`, `ground_truth`</p>
            </div>
            {qaPairs.length > 0 && (
              <div>
                ...
                <div className="mt-2 space-y-2 p-4 border rounded-md max-h-60 overflow-y-auto">
                  ...
                  {qaPairs.map((pair, index) => ( <div key={index} className="flex items-center"><input id={`q-${index}`} type="checkbox" checked={selectedQuestions[index] || false} onChange={() => handleCheckboxChange(index)} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor={`q-${index}`} className="ml-3 block text-sm text-gray-700 truncate">{pair.question}</label></div> ))}
                </div>
              </div>
            )}
        ...
        {results && (
          <div>
            ...
            <div className="bg-white shadow-lg rounded-lg p-6 mb-8">
              <h3 className="text-xl font-bold text-gray-800 mb-4 text-center">総合評価スコア</h3>
              <div className="flex justify-around">
                <div className="text-center">
                  <p className="text-sm text-gray-500">忠実性 (Faithfulness)</p>
                  <p className={`text-4xl font-bold ${getScoreColor(results.final_scores.average_faithfulness)}`}>
                    {(results.final_scores.average_faithfulness * 100).toFixed(1)}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-gray-500">回答の関連性 (Answer Relevancy)</p>
                  <p className={`text-4xl font-bold ${getScoreColor(results.final_scores.average_answer_relevancy)}`}>
                    {(results.final_scores.average_answer_relevancy * 100).toFixed(1)}
                  </p>
                </div>
              </div>
            </div>
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                <table className="min-w-full leading-normal">
                    <thead>
                        <tr>
                            <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">質問</th>
                            <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">AIの回答</th>
                            <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">正解の回答</th>
                            <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">評価</th>
                        </tr>
                    </thead>
                    <tbody>
                        {results.results_by_question.map((item, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                                <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm w-3/12"><p className="text-gray-900">{item.question}</p></td>
                                <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm w-4/12"><p className="text-gray-800">{item.generated_answer}</p></td>
                                <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm w-3/12"><p className="text-green-800 bg-green-50 p-1 rounded">{item.ground_truth}</p></td>
                                <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm text-right w-2/12">
                                  <div className={`text-sm font-semibold ${getScoreColor(item.faithfulness_score)}`}>忠実性: {(item.faithfulness_score * 100).toFixed(0)}</div>
                                  <div className={`text-sm font-semibold ${getScoreColor(item.relevancy_score)}`}>関連性: {(item.relevancy_score * 100).toFixed(0)}</div>
                                </td>
                            </tr>
                        ))}
                    </tbody>
