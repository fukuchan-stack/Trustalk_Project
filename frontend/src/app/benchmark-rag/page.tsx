...
export default function BenchmarkRagPage() {
  ...
  const handleSelectAllAdvancedChange = (event: React.ChangeEvent<HTMLInputElement>) => { const isChecked = event.target.checked; setAdvancedRagOptions({ chunking: isChecked, hybridSearch: isChecked, promptTuning: isChecked }); };
    ...
    <main className="flex min-h-screen flex-col items-center p-4 sm:p-8 bg-gray-50">
      <div className="w-full max-w-6xl">
        ...
        <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200 mb-12">
          ...
          <p className="text-gray-700 mb-6">質問応答データセットとコンテキスト（知識源）のCSVファイルをアップロードし、選択したモデルと技術のRAG性能を評価します。</p>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="qa-file-upload" className="block text-sm font-medium text-gray-700 mb-2">1. 質問・回答CSVファイルを選択</label>
            ...
            <div>
              <label htmlFor="model-select" className="block text-sm font-medium text-gray-700 mb-2">3. 評価するAIモデルを選択</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">4. Advanced RAG 技術を選択 (オプション)</label>
              <div className="mt-2 space-y-3 p-4 border rounded-md bg-gray-50">
                ...
                <div className="flex items-start"><div className="flex items-center h-5"><input id="chunking" type="checkbox" checked={advancedRagOptions.chunking} onChange={() => handleAdvancedRagChange('chunking')} className="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" /></div><div className="ml-3 text-sm"><label htmlFor="chunking" className="font-medium text-gray-700">チャンキング (Chunking)</label><p className="text-gray-500">文脈を適切なサイズに分割して、検索精度を向上させます。</p></div></div>
                <div className="flex items-start"><div className="flex items-center h-5"><input id="hybrid-search" type="checkbox" checked={advancedRagOptions.hybridSearch} onChange={() => handleAdvancedRagChange('hybridSearch')} className="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" /></div><div className="ml-3 text-sm"><label htmlFor="hybrid-search" className="font-medium text-gray-700">ハイブリッド検索 (Hybrid Search)</label><p className="text-gray-500">意味の類似性（ベクトル検索）とキーワードの一致を組み合わせて、関連性の高い文脈を見つけます。</p></div></div>
                <div className="flex items-start"><div className="flex items-center h-5"><input id="prompt-tuning" type="checkbox" checked={advancedRagOptions.promptTuning} onChange={() => handleAdvancedRagChange('promptTuning')} className="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" /></div><div className="ml-3 text-sm"><label htmlFor="prompt-tuning" className="font-medium text-gray-700">プロンプト最適化 (Prompt)</label><p className="text-gray-500">AIへの指示文をより高度なものに切り替え、思考を促すことで回答の精度を高めます。</p></div></div>
              </div>
            </div>
            {qaPairs.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">5. 評価する質問を選択</label>
          ...
          <div>
            ...
            <div className="bg-white shadow-lg rounded-lg p-6 mb-8">
              ...
              <div className="flex justify-around items-center">
                <div className="text-center"><p className="text-base font-medium text-gray-600">忠実性 (Faithfulness)</p><p className={`text-4xl font-bold ${getScoreColor(results.final_scores.average_faithfulness)}`}>{(results.final_scores.average_faithfulness * 100).toFixed(1)}</p></div>
                <div className="text-center"><p className="text-base font-medium text-gray-600">回答の関連性 (Answer Relevancy)</p><p className={`text-4xl font-bold ${getScoreColor(results.final_scores.average_answer_relevancy)}`}>{(results.final_scores.average_answer_relevancy * 100).toFixed(1)}</p></div>
                <div className="text-center border-l-2 border-gray-200 pl-6 ml-6"><p className="text-base font-medium text-gray-600">概算コスト</p><p className="text-4xl font-bold text-gray-800">{results.total_cost.toFixed(3)}<span className="text-2xl font-normal"> 円</span></p></div>
              </div>
            </div>
